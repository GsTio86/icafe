 ...
 * <code>InputStream</code>.  Seeking backwards is supported by means of a file cache.  In
 * circumstances that do not allow the creation of a temporary file (for example, due to security
...
public final class FileCacheSeekableStream extends SeekableStream {
  ...
  /**
   * Constructs a <code>MemoryCacheSeekableStream</code> that takes its source data from a regular
   * <code>InputStream</code>. Seeking backwards is supported by means of an file cache.
   *
   ...
   * The return value is equal to the smaller of <code>pos</code> and the length of the source
   * file.
   */
   ...
   * least one byte. If no byte is available because the stream is at end of file, the value
  ...
  /**
   * Closes this stream and releases any system main.resources associated with the stream.
   *
   * @throws IOException if an I/O error occurs.
   */
