 ...
 * creating drawings with your programs. It uses a simple graphics model that allows you to create
 * drawings consisting of points, lines, and curves in a window on your computer and to save the
 * drawings to a file.
 * <p>
...
public final class StdDraw
  ...
  /**
   * Shade of red used in Algorithms 4th edition. It is Pantone 1805U. The RGB values are
   ...
   * drawing methods such as <tt>line()</tt>, <tt>circle()</tt>, and <tt>square()</tt> will not be
   * displayed on screen until the next call to <tt>show()</tt>. This is useful for producing
  ...
  /**
   * What is the next key that was typed by the user? This method returns a Unicode character
   * corresponding to the key typed (such as 'a' or 'A'). It cannot identify action keys (such as F1
  ...
  /**
   * Is the keycode currently being pressed? This method takes as an argument the keycode
   * (corresponding to a physical key). It can handle action keys (such as F1 and arrow keys) and
   * modifier keys (such as shift and control). See <a href = "http://download.oracle.com/javase/6/docs/api/java/awt/event/KeyEvent.html">KeyEvent.java</a>
