 ...
 * WY    24Nov2017  Added invertBits(short[]) to handle TIFF 16 bit WhiteIsZero WY    07Feb2016
 * Renamed methods related to popularity quantization WY    31Jan2016  Removed ditherThreshold
 * related method arguments WY    31Dec2015  Removed error limit from dither_FloydSteinberg WY
 * 03Nov2015  Bug fix for reduceColors() WY    16Sep2015  Added getScaledInstance() to IMGUtils WY
 *  15Sep2015  Added parameter to ImageParam to set quantization method WY    10Sep2015  Removed
 * ColorEntry from checkColorDepth() WY    05Sep2015  Added ordered dither support for color images
 * WY    03Sep2015  Added ordered dither support for bilevel images WY    03Feb2015  Added
 * createThumbnail() to create a thumbnail from an image WY    27Jan2015  Added
 * createThumbnail8BIM() to wrap a BufferedImage to _8BIM WY    22Jan2015  Factored out
 * guessImageType(byte[]) WY    24Dec2014  Rename CMYK2RGB() to iccp2rgbRaster() WY    17Dec2014
 * Bug fix for rgb2bilevelDither() to bypass transparent pixels WY    03Dec2014  Bug fix for
 * getRGB() to fall back to BufferedImage.getRGB() WY    26Nov2014  Changed rgb2bilevel() to take
 * into account transparency WY    03Nov2014  Added CMYK2RGB() to convert CMYK raster to RGB raster
 * WY    22Sep2014  Added guessImageType() to auto detect image type WY    13Aug2014  Added
 * RGB2YCCK_Inverted() to support YCCK JPEG WY    05May2014  Added getRGB() and getRGB2() to replace
...
public class IMGUtils {
  ...
  private static final float GAMMA = 0.45455f; // Default gamma
  private static final float DISPLAY_EXPONENT = 1.8f; // Default display exponent
   ...
   */
  public static int[] checkColorDepth(int[] rgbTriplets, byte[] newPixels,
  ...
  public static void correctGamma(int[] rgbColorPalette, byte[] gammaTable) {
    for (int i = 0; i < rgbColorPalette.length; i++) {
      ...
      rgbColorPalette[i] =
          ((rgbColorPalette[i] & 0xff000000) | ((red & 0xff) << 16) | ((green & 0xff) << 8) | (blue
              & 0xff));
    }
  }
   ...
   */
  public static void dither_Bayer(byte[] gray, byte[] mask, int width, int height,
      int[][] threshold) {
   ...
   */
  public static void dither_FloydSteinberg(byte[] gray, byte[] mask, int width, int height,
      int threshold) {
    // Define error arrays
   ...
   */
  public static void dither_Bayer(int[] rgbTriplet, int width, int height, byte[] newPixels,
      int no_of_color,
   ...
   */
  public static void dither_FloydSteinberg(int[] rgbTriplet, int width, int height,
      byte[] newPixels, int no_of_color,
  ...
  public static ICC_ColorSpace getICCColorSpace(String pathToICCProfile) throws IOException {
    return new ICC_ColorSpace(
        ICC_Profile.getInstance(IMGUtils.class.getResourceAsStream(pathToICCProfile)));
  }
      ...
      case DataBuffer.TYPE_INT:
        ...
        if (type == BufferedImage.TYPE_INT_ARGB_PRE) {
          for (int i = 0; i < imageSize; i++) {
            ...
            rgbs[i] = (rgbs[i] & 0xff000000) | ((red & 0xff) << 16) | ((green & 0xff) << 8) | (blue
                & 0xff);
          }
      ...
      case DataBuffer.TYPE_BYTE:
        ...
        if (type == BufferedImage.TYPE_BYTE_INDEXED || type == BufferedImage.TYPE_BYTE_BINARY) {
          ...
          indexModel.getAlphas(alphas);
					...
					for (int i = 0, index = 0; i < imageSize; i++) {
						rgbs[i] =
								((0xff000000) | ((bpixels[index++] & 0xff) << 16) | ((bpixels[index++] & 0xff) << 8)
										| (bpixels[index++] & 0xff));
					}
          ...
          for (int i = 0, index = 0; i < imageSize; i++, index += 4) {
            ...
            rgbs[i] =
                (bpixels[index + 3] & 0xff000000) | ((red & 0xff) << 16) | ((green & 0xff) << 8) | (
                    blue & 0xff);
          }
      ...
      case DataBuffer.TYPE_BYTE:
        ...
        if (type == BufferedImage.TYPE_BYTE_INDEXED || type == BufferedImage.TYPE_BYTE_BINARY) {
          ...
          if (numOfBanks == 1) {
						...
						for (int i = 0; i < imageSize; i++) {
							rgbs[i] =
									((bpixels[0][i] & 0xff) << 24) | (bpixels[1][i] & 0xff) | ((bpixels[2][i] & 0xff)
											<< 8) | ((bpixels[3][i] & 0xff) << 16);
						}
          }
        } else if (type == BufferedImage.TYPE_3BYTE_BGR) {
          if (numOfBanks == 1) {
						for (int i = 0, index = 0; i < imageSize; i++) {
							rgbs[i] =
									0xff000000 | (bpixels[0][index++] & 0xff) | ((bpixels[0][index++] & 0xff) << 8)
          ...
          if (numOfBanks == 1) {
            for (int i = 0, index = 0; i < imageSize; i++, index += 4) {
              ...
              rgbs[i] =
                  ((bpixels[0][index] & 0xff) << 24) | ((red & 0xff) << 16) | ((green & 0xff) << 8)
                      | (blue & 0xff);
            }
          } else { // We assume 4 banks
            for (int i = 0; i < imageSize; i++) {
              ...
              rgbs[i] =
                  ((bpixels[0][i] & 0xff) << 24) | ((red & 0xff) << 16) | ((green & 0xff) << 8) | (
                      blue & 0xff);
            }
          }
        } else if (type == BufferedImage.TYPE_BYTE_GRAY) {
					for (int i = 0; i < imageSize; i++) {
						rgbs[i] =
								0xff000000 | ((bpixels[0][i] & 0xff) << 16) | ((bpixels[0][i] & 0xff) << 8) | (
										bpixels[0][i] & 0xff);
					}
        }
        return rgbs;
  ...
  private static int[] reduceColorsPopularity(int[] rgbTriplets, int colorDepth, byte[] newPixels,
    ...
    for (int i = 0; i < no_of_color; i++) {
      ...
      colorPalette[i] =
          ((clrPalAlpha[i] << 24) | (clrPalRed[i] << 16) | (clrPalGreen[i] << 8) | clrPalBlue[i]);
      colorIndex[i] = i;
    }
    ...
    if (colorCount > no_of_color) {
      for (int i = no_of_color; i < colorCount; i++) {
        ...
        err1 = (red - clrPalRed[0]) * (red - clrPalRed[0]) + (green - clrPalGreen[0]) * (green
            - clrPalGreen[0]) +
            (blue - clrPalBlue[0]) * (blue - clrPalBlue[0]) + (alpha - clrPalAlpha[0]) * (alpha
            - clrPalAlpha[0]);

        for (int j = 1; j < no_of_color; j++) {
          err2 = (red - clrPalRed[j]) * (red - clrPalRed[j]) + (green - clrPalGreen[j]) * (green
              - clrPalGreen[j]) +
              (blue - clrPalBlue[j]) * (blue - clrPalBlue[j]) + (alpha - clrPalAlpha[j]) * (alpha
              - clrPalAlpha[j]);
          if (err2 < err1) {
            err1 = err2;
            index = j;
  ...
  // Color quantization
  public static int[] reduceColors(QuantMethod quantMethod, int[] rgbTriplets, int colorDepth,
      byte[] newPixels, final int[] colorPalette, boolean fullAlpha) {
    int[] colorInfo = new int[2];
		if (quantMethod == QuantMethod.WU_QUANT) {
			...
			colorInfo =
					reduceColorsPopularity(rgbTriplets, colorDepth, newPixels, colorPalette, fullAlpha);
		}
   ...
   */
  public static int[] reduceColorsDiffusionDither(int[] rgbTriplets, int width, int height,
    ...
    dither_FloydSteinberg(rgbTriplets, width, height, newPixels, colors, colorPalette,
        colorInfo[1]);
    // Return the actual bits per pixel and the transparent color index if any
  ...
  public static int[] reduceColorsDiffusionDither(QuantMethod quantMethod, int[] rgbTriplets,
      int width, int height, int colorDepth, byte[] newPixels, final int[] colorPalette) {
		if (colorDepth > 8 || colorDepth < 1) {
			throw new IllegalArgumentException("Invalid color depth " + colorDepth);
		}
    int[] colorInfo = new int[2];
  ...
  public static int[] reduceColorsOrderedDither(int[] rgbTriplet, int width, int height,
    ...
    dither_Bayer(rgbTriplet, width, height, newPixels, colors, colorPalette, colorInfo[1],
        threshold);
    // Return the actual bits per pixel and the transparent color index if any
  ...
  public static int[] reduceColorsOrderedDither(QuantMethod quantMethod, int[] rgbTriplets,
      int width, int height, int colorDepth, byte[] newPixels, final int[] colorPalette,
      int[][] threshold) {
		if (colorDepth > 8 || colorDepth < 1) {
			throw new IllegalArgumentException("Invalid color depth " + colorDepth);
    ...
    dither_Bayer(rgbTriplets, width, height, newPixels, colors, colorPalette, colorInfo[1],
        threshold);
    // Return the actual bits per pixel and the transparent color index if any
  ...
  private static int reduceColors(int[] rgbTriplet, int colorDepth, final int[] colorPalette,
      int[] colorInfo) {
		if (colorDepth > 8 || colorDepth < 1) {
			throw new IllegalArgumentException("Invalid color depth " + colorDepth);
		}
  ...
  public static byte[] rgb2bilevelOrderedDither(int[] rgb, int imageWidth, int imageHeight,
      int[][] threshold) {
    // RGB to gray-scale
    byte[] pixels = new byte[rgb.length];
  ...
  public static void RGB2CMYK(ICC_ColorSpace cmykColorSpace, int[] rgb, float[][] C, float[][] M,
    ...
    BufferedImage rgbImage =
        new BufferedImage(new DirectColorModel(24, 0x00ff0000, 0x0000ff00, 0x000000ff), raster,
            false, null);
  ...
  public static byte[] RGB2CMYK(ICC_ColorSpace cmykColorSpace, int[] rgb, int imageWidth,
      int imageHeight, boolean hasAlpha) {
    DataBuffer db = new DataBufferInt(rgb, rgb.length);
    ...
    WritableRaster raster =
        Raster.createPackedRaster(db, imageWidth, imageHeight, imageWidth, bandMasks, null);
  ...
  public static void RGB2CMYK_Inverted(ICC_ColorSpace cmykColorSpace, int[] rgb, float[][] C,
      float[][] M, float[][] Y, float[][] K, int imageWidth, int imageHeight) {
    DataBuffer db = new DataBufferInt(rgb, rgb.length);
    WritableRaster raster = Raster.createPackedRaster(db, imageWidth, imageHeight, imageWidth,
        new int[] {0x00ff0000, 0x0000ff00, 0x000000ff}, null);
    ColorSpace sRGB = ColorSpace.getInstance(ColorSpace.CS_sRGB);
  ...
  public static float[][] rgb2grayscale(int[] rgb, int imageWidth, int imageHeight) {
    ...
    for (int i = 0, index = 0; i < imageHeight; i++) {
      for (int j = 0; j < imageWidth; j++, index++) {
        grayscale[i][j] = (float) (
            ((rgb[index] >> 16) & 0xff) * 0.2126 + ((rgb[index] >> 8) & 0xff) * 0.7152
  ...
  public static void RGB2YCbCr(int[] rgb, float[][] Y, float[][] Cb, float[][] Cr, int imageWidth,
      int imageHeight) {
  ...
  // Convert RGB to YCbCr with level shift (minus 128)
  public static void RGB2YCbCr(int[][] red, int[][] green, int[][] blue, float[][] Y, float[][] Cb,
      float[][] Cr, int imageWidth, int imageHeight) {
    //
    for (int i = 0; i < imageHeight; i++) {
      for (int j = 0; j < imageWidth; j++) {
  ...
  public static void RGB2YCCK_Inverted(ICC_ColorSpace cmykColorSpace, int[] rgb, float[][] Y,
      float[][] Cb, float[][] Cr, float[][] K, int imageWidth, int imageHeight) {
    DataBuffer db = new DataBufferInt(rgb, rgb.length);
    WritableRaster raster = Raster.createPackedRaster(db, imageWidth, imageHeight, imageWidth,
        new int[] {0x00ff0000, 0x0000ff00, 0x000000ff}, null);
    ColorSpace sRGB = ColorSpace.getInstance(ColorSpace.CS_sRGB);
