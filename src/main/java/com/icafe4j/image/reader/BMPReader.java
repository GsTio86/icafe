...
public class BMPReader extends ImageReader {
  ...
  public BufferedImage read(InputStream is) throws Exception {
    ...
    LOGGER.info("Scanline alignment: {}",
        ((alignment == BMPOptions.ALIGN_BOTTOM_UP) ? "BOTTOM_UP" : "TOP_DOWN"));
    ...
    if (bitPerWidth % 32
        == 0) { // To make sure scan lines are padded out to even 4-byte boundaries.
    ...
    switch (bitmapHeader.bitCount) {
      ...
      case 4:
      case 8:
        if (compression == BmpCompression.BI_RLE4.getValue()
            || compression == BmpCompression.BI_RLE8.getValue()) {
  ...
  private BufferedImage read24bitTrueColorBitmap(InputStream is) throws Exception {
    ...
    if (alignment == BMPOptions.ALIGN_BOTTOM_UP) {
      for (int i = 0, startIndex = (height - 1) * bytePerWidth; i < height;
          i++, startIndex -= bytePerWidth) {
    ...
    WritableRaster raster =
        Raster.createInterleavedRaster(db, width, height, bytePerWidth, numOfBands, off, null);
    ColorModel cm =
        new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), false, false, trans,
    ...
    if (alignment == BMPOptions.ALIGN_BOTTOM_UP) {
      for (int i = 1, index = 0; i <= height; i++) {
        ...
        for (int j = 0, nindex = 0; j < width; j++) {
          pix[index++] =
              ((brgb[nindex++] & 0xff) | ((brgb[nindex++] & 0xff) << 8) | ((brgb[nindex++] & 0xff)
                  << 16) | (0xff << 24));
          nindex++;
        }
      }
  ...
  private BufferedImage read32bitTrueColorBitmap2(InputStream is) throws Exception {
    ...
    WritableRaster raster =
        Raster.createInterleavedRaster(db, width, height, bytePerScanLine, numOfBands, off, null);
    ColorModel cm =
        new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, true, false,
  ...
  private BufferedImage readCompressedIndexColorBitmap(InputStream is) throws Exception {
    ...
    ColorModel cm =
        new IndexColorModel(bitsPerPixel, rgbColorPalette.length, rgbColorPalette, 0, false, -1,
            DataBuffer.TYPE_BYTE);
    ...
    switch (bitsPerPixel) {
      ...
      default:
        throw new IllegalArgumentException(
            "Invalid bitsPerPixel: " + bitsPerPixel + " for BMP indexColor image!");
    }
  ...
  private static class BitmapHeader {
    ...
    void readHeader(InputStream is) throws Exception {
      ...
      fileSize =
          (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
              | ((bhdr[nindex++] & 0xff) << 24);
      ...
      infoHeaderLen =
          (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
              | ((bhdr[nindex++] & 0xff) << 24);
      bhdr =
          new byte[infoHeaderLen - 4]; // infoHeaderLen is different for different bitmap versions
