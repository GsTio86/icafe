 ...
 * WY    18Nov2015  Bug fix to fully skip the chunk if not interested WY    25Dec2014  Added iCCP
 * chunk support for RGB images
 */
...
public class PNGReader extends ImageReader {
  ...
  private static void apply_defilter(InputStream bis, byte[] pixBytes, int height,
      int bytesPerPixel, int bytesPerScanLine) throws Exception {
  ...
  private void correctGamma(int[] image, int width, int height) {
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++, p_index++) {
        ...
        image[p_index] =
            ((image[p_index] & 0xff000000) | ((red & 0xff) << 16) | ((green & 0xff) << 8) | (blue
                & 0xff));
      }
    }
  }
  ...
  private void createUShortGammaTable(float gamma, float displayExponent) {
    ...
    for (int i = 0; i < size; i++) {
      gammaUShortTable[i] =
          (short) (Math.pow((double) i / (size - 1), decodingExponent) * (size - 1));
    }
  }
      ...
      if (alpha != null) { // Deal with single color transparency
        for (int j = 0, s_index = 0, k = 0; j < block_height; j++) {
          for (int i = 0; i < block_width; i++, p_index += x_inc) {
            s_index = p_index << 1;
            spixels[s_index] =
                (short) ((pix_interlaced[k++] & 0xff) << 8 | (pix_interlaced[k++] & 0xff));
  ...
  private short[] generate16BitRGBInterlacedPixels(byte[] compr_data, boolean fullAlpha)
      throws Exception {
    int bytesPerPixel = 0;
    int p_index = 0;
    byte[] pix_interlaced;
      ...
      if (fullAlpha) {
        for (int j = 0, s_index = 0, k = 0; j < block_height; j++) {
          for (int i = 0; i < block_width; i++, p_index += x_inc) {
            s_index = p_index << 2;
            spixels[s_index++] =
                (short) ((pix_interlaced[k++] & 0xff) << 8 | (pix_interlaced[k++] & 0xff)); // Red
            spixels[s_index++] =
                (short) ((pix_interlaced[k++] & 0xff) << 8 | (pix_interlaced[k++] & 0xff)); // Green
            spixels[s_index++] =
                (short) ((pix_interlaced[k++] & 0xff) << 8 | (pix_interlaced[k++] & 0xff)); // Blue
            spixels[s_index++] =
                (short) ((pix_interlaced[k++] & 0xff) << 8 | (pix_interlaced[k++] & 0xff)); // Alpha
          }
          p_index = ((j + 1) * y_inc + y_start) * width + x_start;
        }
  ...
  private short[] generate16BitRGBPixels(byte[] compr_data, boolean fullAlpha) throws Exception {
    ...
    if (alpha != null) { // Deal with single color transparency
      ...
      for (int i = 0, index = 0; i < pixBytes.length; index += 4) {
        ...
        if (spixels[index] == redMask && spixels[index + 1] == greenMask
            && spixels[index + 2] == blueMask) {
  ...
  private byte[] generate8BitRGBInterlacedPixels(byte[] compr_data, boolean fullAlpha)
      throws Exception {
    int bytesPerPixel = 0;
    int p_index = 0;
    byte[] pix_interlaced;
  ...
  private void generateGrayscaleInterlacedPixels(byte[] pixels, byte[] pix_interlaced,
      int block_width, int block_height, int padding, int p_index, int x_start, int y_start,
      int x_inc, int y_inc) {
    //
  ...
  private void generateIndexedInterlacedPixels(byte[] pixels, byte[] pix_interlaced,
      int block_width, int block_height, int padding, int p_index, int x_start, int y_start,
      int x_inc, int y_inc) {
    //
    int i = 0, safeEnd = block_width - padding;
  ...
  private byte[] process_grayscaleAlphaImage(byte[] compr_data) throws Exception {
    ...
    switch (bitsPerPixel) {
      ...
      case 16:
        ...
        break;
      default:
        LOGGER.error("... " + bitsPerPixel
            + " bit color depth is invalid for full alpha grayscale image...");
    }
  ...
  private byte[] process_grayscaleAlphaInterlacedImage(byte[] compr_data) throws Exception {
    ...
    switch (bitsPerPixel) {
      ...
      default:
        LOGGER.error("... " + bitsPerPixel
            + " bit color depth is not valid for grayscale full alpha image...");
    }
  ...
  private byte[] process_grayscaleImage(byte[] compr_data) throws Exception {
    ...
    switch (bitsPerPixel) {
      ...
      default:
        LOGGER.error(
            "... " + bitsPerPixel + " bit color depth is not valid for grayscale image...");
    }
    // Now inflate the data.
    pixBytes = new byte[height * bytesPerScanLine];
    // Wrap an InflaterInputStream with a bufferedInputStream to speed up reading
    ...
    switch (ColorType.fromInt(color_format)) {
      case GRAY_SCALE:
        ...
        if (bitsPerPixel == 16) {
          ...
          cm = new IndexColorModel(bitsPerPixel, rgbColorPalette.length, rgbColorPalette, 0, true,
      ...
      case GRAY_SCALE_WITH_ALPHA:
        ...
        if (bitsPerPixel == 16) {
          ...
          raster =
              Raster.createInterleavedRaster(db, width, height, bytesPerScanLine, 2, off, null);
      ...
      case TRUE_COLOR:
        if (bitsPerPixel == 16) {
          ...
          db = new DataBufferUShort(spixels, spixels.length);
          raster =
              Raster.createInterleavedRaster(db, width, height, width * numOfBands, numOfBands, off,
                  null);
          cm = new ComponentColorModel(colorSpace, nBits, hasAlpha, false, trans,
              DataBuffer.TYPE_USHORT);
          if (hasICCP) {
            raster = IMGUtils.iccp2rgbRaster(raster, cm);
            cm =
                new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, hasAlpha,
                    false, trans, DataBuffer.TYPE_USHORT);
          }
        } else {
          if (interlace_method == NON_INTERLACED) {
      ...
      case TRUE_COLOR_WITH_ALPHA:
        if (bitsPerPixel == 16) {
          ...
          raster =
              Raster.createInterleavedRaster(db, width, height, width * numOfBands, numOfBands, off,
                  null);
          cm = new ComponentColorModel(colorSpace, nBits, true, false, trans,
              DataBuffer.TYPE_USHORT);
          if (hasICCP) {
            raster = IMGUtils.iccp2rgbRaster(raster, cm);
  ...
  public BufferedImage read(InputStream is) throws Exception {
    ...
    LOGGER.info("image compression: {} - {}", compression,
        PNGDescriptor.getCompressionTypeDescrition(compression));
    LOGGER.info("image filter method: {} - {}", filter_method,
        PNGDescriptor.getFilterTypeDescription(filter_method));
  ...
  private void read_IDAT(InputStream is, int data_len, ByteArrayOutputStream compr_data)
      throws Exception {
