...
public class PCXReader extends ImageReader {
  ...
  private void readPalette(byte[] buf) throws Exception {
    ...
    for (int i = 0; i < buf.length; i += 3) {
      rgbColorPalette[index++] =
          ((0xff << 24) | ((buf[nindex++] & 0xff) << 16) | ((buf[nindex++] & 0xff) << 8) | (
              buf[nindex++] & 0xff));
    }
  }
  ...
  private BufferedImage readTrueColorPcx(InputStream is) throws Exception {
    ...
    ColorModel cm =
        new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, false, false,
...
// Label to break
  ...
  private BufferedImage read256ColorPcx(InputStream is) throws Exception {
    ...
    WritableRaster raster =
        Raster.createInterleavedRaster(db, width, height, bytesPerLine, 1, off, null);
