 ...
 * WY    03Jan2018  Fix issue with fillOrder 2 WY    07Dec2017  Added support for CCITTRLE
 * compression WY    28Nov2017  Added gray-scale alpha support WY    23Nov2017  Added support for 16
 * bits gray-scale WY    22Nov2017  Added support for BlackIsZero and WhiteIsZero WY    09Nov2015
 * Fixed bug with stripped CMYK decoding WY    13Sep2015  Extract unpackStrip() method WY
 * 08Jan2015  Better exception handling to resume from failed frame decoding WY    06Jan2015
 * Enhancement to decode multipage TIFF WY    10Dec2014  Fixed bug for bitsPerSample%8 != 0 case to
 * assume BIG_ENDIAN WY    09Dec2014  Added support for LSB2MSB FillOrder for RGB image WY
 * 07Dec2014  Added support for floating point sample data type WY    03Dec2014  Code clean and test
 * showing float type raster images WY    01Dec2014  Added support for less than 8 bits planar
 * stripped image WY    28Nov2014  Added support for 32 bits tiled image WY    14Nov2014  Added
 * support for tiled Palette Image WY    07Nov2014  Added support for 16 bit CMYK image WY
 * 06Nov2014  Planar support for stripped YCbCr image WY    05Nov2014  Fixed bug for YCbCr image
 * with wrong image width and height WY    31Oct2014  Added basic support for uncompressed and LZW
 * compressed YCbCr WY    15Oct2014  Added basic support for 16 bits RGB image WY    14Oct2014
 * Revised to show specification violation TIFF LZW compression WY    14Oct2014  Revised to show RGB
 * TIFF with extra sample tag
 */
...
public class TIFFReader extends ImageReader {
  ...
  private static final int[] FOUR_COLOR_PALETTE_WHITE_IS_ZERO =
      {0xFFFFFFFF, 0xFF808080, 0xFF404040, 0xFF000000};
  ...
  static {
    for (int i = 0; i < 256; i++) {
      EIGHT_BIT_COLOR_PALETTE_WHITE_IS_ZERO[255 - i] =
          0xFF000000 | (i << 16) | (i << 8) | (i & 0xff);
    }
  }
  ...
  private static byte[] applyDePredictor(int numOfSamples, byte[] input, int imageWidth,
      int imageHeight) {
    for (int i = 0, inc = numOfSamples * imageWidth, maxVal = inc - numOfSamples, minVal =
        numOfSamples; i <= imageHeight - 1; maxVal += inc, minVal += inc, i++) {
  ...
  private static short[] applyDePredictor(int numOfSamples, short[] input, int imageWidth,
      int imageHeight) {
    for (int i = 0, inc = numOfSamples * imageWidth, maxVal = inc - numOfSamples, minVal =
        numOfSamples; i <= imageHeight - 1; maxVal += inc, minVal += inc, i++) {
      for (int j = minVal; j <= maxVal; j += numOfSamples) {
  ...
  private BufferedImage decodeStrippedTiff(IFD ifd) throws Exception {
    ...
    TiffFieldEnum.PlanarConfiguration e_planaryConfiguration =
        TiffFieldEnum.PlanarConfiguration.fromValue(planaryConfiguration);
    ...
    if (f_sampleFormat != null
        && f_sampleFormat.getDataAsLong()[0] == 3) { // Floating point sample data type
      floatSample = true;
    ...
    switch (e_photoMetric) {
      case PALETTE_COLOR:
        ...
        for (int i = 0, index = 0; i < colorMap.length / 3; i++) {
          rgbColorPalette[index++] =
              0xff000000 | ((colorMap[i] & 0xff00) << 8) | ((colorMap[i + numOfColors] & 0xff00))
        ...
        if (bitsPerSample != 8) {
          ...
          raster =
              Raster.createInterleavedRaster(db, imageWidth, imageHeight, imageWidth, 1, off, null);
        }
        ColorModel cm =
            new IndexColorModel(bitsPerSample, rgbColorPalette.length, rgbColorPalette, 0, false,
      ...
      case SEPARATED:
        ...
        if (samplesPerPixel >= 5) {
          ...
          nBits =
              new int[] {bitsPerSample, bitsPerSample, bitsPerSample, bitsPerSample, bitsPerSample};
        ...
        if (bitsPerSample == 16) {
          ...
          if (planaryConfiguration == 2) {
            ...
            raster = Raster.createBandedRaster(db, imageWidth, imageHeight,
                bytesPerScanLine * 8 / bitsPerSample, bankIndices, bandoff, null);
          } else {
            raster =
                Raster.createInterleavedRaster(db, imageWidth, imageHeight, imageWidth * numOfBands,
                    numOfBands, bandoff, null);
          }
          if (profile != null) {
            raster = IMGUtils.iccp2rgbRaster(raster, cm);
            cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits,
                transparent, isAssociatedAlpha, trans, raster.getTransferType());
          }
        } else {
          cm = new ComponentColorModel(colorSpace, nBits, transparent, isAssociatedAlpha, trans,
          ...
          if (profile != null) {
            raster = IMGUtils.iccp2rgbRaster(raster, cm);
            cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits,
                transparent, isAssociatedAlpha, trans, DataBuffer.TYPE_BYTE);
          }
        }
      ...
      case YCbCr:
        ...
        int expandedImageWidth =
            ((imageWidth + samplingFactor[0] - 1) / samplingFactor[0]) * samplingFactor[0];
        int expandedImageHeight =
            ((imageHeight + samplingFactor[1] - 1) / samplingFactor[1]) * samplingFactor[1];
        ...
        if (planaryConfiguration == 1) {
          ...
          switch (compression) {
            case NONE:
              for (int i = 0; i < stripByteCounts.length; i++) {
                ...
                offsetY = upsampling(offsetY, numOfDataUnit, bytesPerUnitY, samplingFactor,
                    referenceBlackY, referenceWhiteY, referenceBlackCb,
                    referenceWhiteCb, referenceBlackCr, referenceWhiteCr, codingRangeY,
                    codingRangeCbCr, lumaRed, lumaGreen, lumaBlue, temp,
            ...
            case LZW:
              for (int i = 0; i < stripByteCounts.length; i++) {
                ...
                offsetY = upsampling(offsetY, numOfDataUnit, bytesPerUnitY, samplingFactor,
                    referenceBlackY, referenceWhiteY, referenceBlackCb,
                    referenceWhiteCb, referenceBlackCr, referenceWhiteCr, codingRangeY,
                    codingRangeCbCr, lumaRed, lumaGreen, lumaBlue, temp2,
                    pixels, expandedImageWidth, dataUnitsPerWidth);
              }
          ...
          if (stripByteCounts.length == 1 && samplesPerPixel != 1) {
            throw new RuntimeException(
                "stripByteCounts length 1 is not consistent with samplesPerPixel "
                    + samplesPerPixel);
          }
      ...
      case RGB:
        ...
        if (planaryConfiguration == 2) {
          ...
          if (floatSample) { // Floating point sample data type
            ...
            cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits,
                transparent, isAssociatedAlpha, trans, dataBufferType);
            // Create a TYPE_FLOAT sample model (specifying how the pixels are stored)
            SampleModel sampleModel =
                new BandedSampleModel(dataBufferType, imageWidth, imageHeight, imageWidth,
                    bankIndices, bandoff);
            ...
            if (bitsPerSample == 16) {
              ...
              for (int i = 0; i < samplesPerPixel; i++) {
                ints[i] =
                    (int[]) (ArrayUtils.toNBits(bitsPerSample, rgb[i], samplesPerPixel * imageWidth,
                        bigEndian));
              }
            ...
            cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits,
                transparent, isAssociatedAlpha, trans, dataBufferType);
            raster = Raster.createBandedRaster(db, imageWidth, imageHeight, imageWidth, bankIndices,
                bandoff, null);
          }
        } else {
          if (floatSample) {
            if (bitsPerSample >= 16 && bitsPerSample <= 32) {
              if (bitsPerSample % 8 == 0) {
                ...
                cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits,
                    transparent, isAssociatedAlpha, trans, DataBuffer.TYPE_FLOAT);
                db = new DataBufferFloat(tempArray, tempArray.length);
                // Create a TYPE_FLOAT sample model (specifying how the pixels are stored)
                SampleModel sampleModel =
                    new PixelInterleavedSampleModel(DataBuffer.TYPE_FLOAT, imageWidth, imageHeight,
                        samplesPerPixel, imageWidth * samplesPerPixel, bandoff);
                raster = Raster.createWritableRaster(sampleModel, db, null);
              } else {
              ...
              cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits,
                  transparent, isAssociatedAlpha, trans, DataBuffer.TYPE_DOUBLE);
              db = new DataBufferDouble(tempArray, tempArray.length);
              // Create a TYPE_FLOAT sample model (specifying how the pixels are stored)
              SampleModel sampleModel =
                  new PixelInterleavedSampleModel(DataBuffer.TYPE_DOUBLE, imageWidth, imageHeight,
                      samplesPerPixel, imageWidth * samplesPerPixel, bandoff);
              raster = Raster.createWritableRaster(sampleModel, db, null);
            } else {
              throw new UnsupportedOperationException("Unsupported bit depth: " + bitsPerSample);
            ...
            if (bitsPerSample < 8) {
              Object tempArray =
                  ArrayUtils.toNBits(bitsPerSample * samplesPerPixel, pixels, imageWidth, true);
              ...
              cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits,
                  transparent, isAssociatedAlpha, trans, DataBuffer.TYPE_BYTE);
              db = new DataBufferByte(pixels, pixels.length);
              raster = Raster.createInterleavedRaster(db, imageWidth, imageHeight,
                  imageWidth * numOfBands, numOfBands, bandoff, null);
            } else {
              Object tempArray =
                  ArrayUtils.toNBits(bitsPerSample, pixels, samplesPerPixel * imageWidth,
                      bitsPerSample % 8 != 0 || endian == IOUtils.BIG_ENDIAN);
              if (bitsPerSample <= 16) {
                cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits,
                    transparent, isAssociatedAlpha, trans, DataBuffer.TYPE_USHORT);
              } else if (bitsPerSample
                  == 32) { // A workaround for Java's ComponentColorModel bug with 32 bit sample
                cm = new Int32ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB),
                    transparent);
              } else {
                cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits,
                    transparent, isAssociatedAlpha, trans, DataBuffer.TYPE_INT);
              }
              raster = cm.createCompatibleWritableRaster(imageWidth, imageHeight);
              raster.setDataElements(0, 0, imageWidth, imageHeight, tempArray);
            }
      ...
      case WHITE_IS_ZERO:
          ...
          case CCITTFAX3:
            ...
            if ((t4Options & GROUP3OPT_UNCOMPRESSED) == GROUP3OPT_UNCOMPRESSED) {
              throw new UnsupportedCompressionException(
                  "Group 3 Uncompressed mode is not supported");
            } else if ((t4Options & GROUP3OPT_2DENCODING) == GROUP3OPT_2DENCODING) {
              decoder = new G32DDecoder(imageWidth,
                  rowsPerStrip); // 2D encoding, need to take care of fill bit
            } else {
              decoder = new G32DDecoder(imageWidth, rowsPerStrip,
                  true); // 1D encoding, need to take care of fill bit
            }
            break;
          case LZW:
            decoder = new LZWTreeDecoder(8, true);
        ...
        if (bitsPerSample <= 8) {
          ...
          db = new DataBufferByte(pixels, pixels.length);
          cm = new IndexColorModel(bitsPerSample, rgbColorPalette.length, rgbColorPalette, 0, false,
              -1, DataBuffer.TYPE_BYTE);
          raster = Raster.createPackedRaster(db, imageWidth, imageHeight, bitsPerSample, null);
          if (samplesPerPixel == 2) { // Deal with alpha transparency
            if (e_photoMetric == PhotoMetric.WHITE_IS_ZERO) {
              IMGUtils.invertBits(pixels,
                  2); // Unlike samplesPerPixel == 1 case, we have to invert bits here as we are using ComponentColorModel
            }
            numOfBands = samplesPerPixel;
          ...
          short[] tempArray =
              (short[]) ArrayUtils.toNBits(bitsPerSample, pixels, samplesPerPixel * imageWidth,
                  bitsPerSample % 8 != 0 || endian == IOUtils.BIG_ENDIAN);
          if (predictor == 2 && planaryConfiguration == 1) {
          ...
          cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_GRAY), transparent,
              isAssociatedAlpha, trans, DataBuffer.TYPE_USHORT);
          raster = cm.createCompatibleWritableRaster(imageWidth, imageHeight);
          raster.setDataElements(0, 0, imageWidth, imageHeight, tempArray);
        } else if (bitsPerSample
            == 64) { // We can't use ColorSpace.CS_GRAY for some reason. Use CS_sRGB instead
          double[] tempArray = ArrayUtils.toDoubleArray(pixels, endian == IOUtils.BIG_ENDIAN);
          cm =
              new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), null, transparent,
                  isAssociatedAlpha, trans, DataBuffer.TYPE_DOUBLE);
          db = new DataBufferDouble(tempArray, tempArray.length);
          SampleModel sampleModel =
              new PixelInterleavedSampleModel(DataBuffer.TYPE_DOUBLE, imageWidth, imageHeight,
                  samplesPerPixel, imageWidth * samplesPerPixel, new int[] {0, 0, 0});
          raster = Raster.createWritableRaster(sampleModel, db, null);
        } else {
          throw new UnsupportedOperationException("Unsupported bit depth: " + bitsPerSample);
        }
    ...
    switch (e_photoMetric) {
      case PALETTE_COLOR:
        ...
        cm = new IndexColorModel(bitsPerSample, rgbColorPalette.length, rgbColorPalette, 0, false,
            -1, DataBuffer.TYPE_BYTE);
        if (bitsPerSample < 8) {
          raster = Raster.createPackedRaster(db, tileWidth * tilesAcross, tileLength * tilesDown,
              bitsPerSample, null);
        } else {
          int[] off = {0}; //band offset, we have only one band start at 0
          if (bitsPerSample > 8) {
            ...
            db = new DataBufferUShort(spixels, spixels.length);
            cm = new IndexColorModel(bitsPerSample, rgbColorPalette.length, rgbColorPalette, 0,
                false, -1, DataBuffer.TYPE_USHORT);
          }
          raster =
              Raster.createInterleavedRaster(db, tileWidth * tilesAcross, tileLength * tilesDown,
            ...
            for (int i = 0; i < tileByteCounts.length; i++) {
              ...
              if (bitsPerSample == 16) {
                ...
                WritableRaster tileRaster =
                    Raster.createPackedRaster(tileDataBuffer, tileWidth, tileLength, bitsPerSample,
                        null);
            ...
            if (planaryConfiguration == 1) {
              for (int i = 0; i < tileByteCounts.length; i++) {
                ...
                if (floatSample) {
                  ...
                  if (bitsPerSample < 8) {
                    ...
                    Object tempArray =
                        ArrayUtils.toNBits(bitsPerSample, temp, samplesPerPixel * tileWidth,
                            endian == IOUtils.BIG_ENDIAN);
                    raster.setDataElements(xoff, yoff, tileWidth, tileLength, tempArray);
                  } else {
                    Object tempArray =
                        ArrayUtils.toNBits(bitsPerSample, temp, samplesPerPixel * tileWidth, true);
                    raster.setDataElements(xoff, yoff, tileWidth, tileLength, tempArray);
                  }
                }
                xoff += tileWidth;
              ...
              for (int i = 0; i < tilesPerImage; i++) {
                ...
                if (floatSample) { // Floating point sample data type
                  if (bitsPerSample >= 16 && bitsPerSample <= 32) {
                    ...
                    if (bitsPerSample == 16) {
                      for (int j = 0; j < samplesPerPixel; j++) {
                        floats[j] =
                            ArrayUtils.to16BitFloatArray(rgb[j], endian == IOUtils.BIG_ENDIAN);
                      }
                    } else if (bitsPerSample == 24) {
                      for (int j = 0; j < samplesPerPixel; j++) {
                        floats[j] =
                            ArrayUtils.to24BitFloatArray(rgb[j], endian == IOUtils.BIG_ENDIAN);
                      }
                    } else if (bitsPerSample == 32) {
                      for (int j = 0; j < samplesPerPixel; j++) {
                  ...
                  // Create a TYPE_FLOAT sample model (specifying how the pixels are stored)
                  SampleModel sampleModel =
                      new BandedSampleModel(dataBufferType, tileWidth, tileLength, tileWidth,
                          bankIndices, bandoff);
                  tileRaster = Raster.createWritableRaster(sampleModel, tileDatabuffer, null);
                } else {
                  if (bitsPerSample == 16) {
                    tileDatabuffer = new DataBufferUShort(new short[][] {
                        ArrayUtils.toShortArray(rgb[0], endian == IOUtils.BIG_ENDIAN),
                        ArrayUtils.toShortArray(rgb[1], endian == IOUtils.BIG_ENDIAN),
                        ArrayUtils.toShortArray(rgb[2], endian == IOUtils.BIG_ENDIAN)},
                        tileWidth * tileLength);
                  } else if (bitsPerSample > 16 && bitsPerSample <= 32) {
                    boolean bigEndian = bitsPerSample % 8 != 0 || endian == IOUtils.BIG_ENDIAN;
                    tileDatabuffer = new DataBufferInt(new int[][] {
                      ...
                      == 64) { // Have to convert to 32 bit since there is no DataBuffer.TYPE_LONG
                    tileDatabuffer = new DataBufferInt(new int[][] {
                        ArrayUtils.to32BitsLongArray(rgb[0], endian == IOUtils.BIG_ENDIAN),
                  ...
                  tileRaster =
                      Raster.createBandedRaster(tileDatabuffer, tileWidth, tileLength, tileWidth,
                          bankIndices, bandoff, null);
                }
  ...
  private void unpackStrip(byte[] pixels, int offset, int bytes2Read, int stripOffset,
      int stripByteCount) throws IOException {
  ...
  private int upsampling(int offsetY, int numOfDataUnit, int bytesPerUnitY, int[] samplingFactor,
      ...
      float referenceBlackCb, float referenceWhiteCb, float referenceBlackCr,
      float referenceWhiteCr, float codingRangeY,
      float codingRangeCbCr, float lumaRed, float lumaGreen, float lumaBlue, byte[] temp,
      byte[] pixels,
    ...
    for (int j = 1; j <= numOfDataUnit; j++) {
      ...
      for (int k = 0; k < samplingFactor[1]; k++) {// Rows
        for (int l = 0; l < samplingFactor[0]; l++, offsetX++) { // Columns
          ...
          float fCb =
              (Cb - referenceBlackCb) * codingRangeCbCr / (referenceWhiteCb - referenceBlackCb);
          float fCr =
              (Cr - referenceBlackCr) * codingRangeCbCr / (referenceWhiteCr - referenceBlackCr);
          /*
           * R, G, and B may be computed from YCbCr as follows:
           * R = Cr * ( 2 - 2 * LumaRed ) + Y
           * G = ( Y - LumaBlue * B - LumaRed * R ) / LumaGreen
