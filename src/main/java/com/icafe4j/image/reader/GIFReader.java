 ...
 * WY    14Oct2015  Fixed bug with transparent color WY    08Oct2015  Removed frame specific methods
 * WY    08Oct2015  Added getGIFFrames() WY    16Feb2015  Removed unnecessary variables - flags and
 * flags2 WY    02Jan2015  Added getFrames() and getFrameCount() for animated GIF WY    18Nov2014
 * Fixed getFrameAsBufferedImageEx() bug with disposal method "RESTORE_TO_PREVIOUS" WY    03Oct2014
 * Added getFrameAsBufferedImageEx()
 */
...
public class GIFReader extends ImageReader {
  ...
  private byte[] decodeLZWInterLaced(InputStream is) throws Exception {
    ...
    int[] passInc =
        {width * passParam[1], width * passParam[3], width * passParam[5], width * passParam[7]};
    int[] passHeight =
        {((height - 1) >> 3) + 1, ((height + 3) >> 3), ((height + 1) >> 2), ((height) >> 1)};
  ...
  protected BufferedImage getFrameAsBufferedImage(InputStream is) throws Exception {
    ...
    ColorModel cm =
        new IndexColorModel(bitsPerPixel, rgbColorPalette.length, rgbColorPalette, 0, false,
  ...
  private byte[] readFrame(InputStream is) throws Exception {
    ...
    do {
      ...
      if (image_separator == 0x21) // (!) Extension Block
      {
        ...
        if (func
            == 0xf9) { // Graphic Control Label - identifies the current block as a Graphic Control Extension
  ...
  private void readGlobalPalette(InputStream is, int num_of_color) throws Exception {
    ...
    for (int i = 0; i < num_of_color; i++) {
      globalColorPalette[i] =
          ((255 << 24) | ((brgb[index1++] & 0xff) << 16) | ((brgb[index1++] & 0xff) << 8) | (
              brgb[index1++] & 0xff));
    }
  }
  ...
  private static class GifHeader {
    ...
    private final byte[] signature = new byte[3];
