  ...
  protected byte[] buildData() {
    ...
    switch (chunkType) {
      case TEXT:
        sb.append(this.text);
        try {
          data = sb.toString().getBytes(StandardCharsets.ISO_8859_1);
        } catch (Exception ex) {
          ex.printStackTrace();
        }
        break;
      case ZTXT:
        try {
          ...
          sb.append('\0');
          bo.write(sb.toString().getBytes(StandardCharsets.ISO_8859_1));
          DeflaterOutputStream ds = new DeflaterOutputStream(bo);
          OutputStreamWriter or = new OutputStreamWriter(ds, StandardCharsets.ISO_8859_1);
      ...
      case ITXT:
        try {
          ...
          if (compressed) {
            ...
            or = new OutputStreamWriter(new DeflaterOutputStream(bo), StandardCharsets.UTF_8);
          } else {
            bo.write(new byte[] {0, 0, 0, 0});
            or = new OutputStreamWriter(bo, StandardCharsets.UTF_8);
          }
          BufferedWriter br = new BufferedWriter(or);
          br.write(this.text);
          br.flush();
