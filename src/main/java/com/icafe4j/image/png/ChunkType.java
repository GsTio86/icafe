...
public enum ChunkType {
  ...
  IDAT("IDAT", 0x49444154, Attribute.CRITICAL,
      60), // PNG data, could have multiple but must appear consecutively
  ...
  TRNS("tRNS", 0x74524E53, Attribute.ANCILLARY,
      50), // Must precede the first IDAT chunk and must follow the PLTE chunk
  GAMA("gAMA", 0x67414D41, Attribute.ANCILLARY,
      30), // Must precede the first IDAT chunk and the PLTE chunk if present
  CHRM("cHRM", 0x6348524D, Attribute.ANCILLARY,
      30), // Must precede the first IDAT chunk and the PLTE chunk if present
  SRGB("sRGB", 0x73524742, Attribute.ANCILLARY,
      30), // Must precede the first IDAT chunk and the PLTE chunk if present
  ICCP("iCCP", 0x69434350, Attribute.ANCILLARY,
      30), // Must precede the first IDAT chunk and the PLTE chunk if present
  BKGD("bKGD", 0x624B4744, Attribute.ANCILLARY,
      50), // Must precede the first IDAT chunk and must follow the PLTE chunk
  PHYS("pHYs", 0x70485973, Attribute.ANCILLARY, 30), // Must precede the first IDAT chunk
  SBIT("sBIT", 0x73424954, Attribute.ANCILLARY,
      30), // Must precede the first IDAT chunk and the PLTE chunk if present
  SPLT("sPLT", 0x73504C54, Attribute.ANCILLARY, 30), // Must precede the first IDAT chunk
  HIST("hIST", 0x68495354, Attribute.ANCILLARY,
      50), // Must precede the first IDAT chunk and must follow the PLTE chunk
  ...
  ChunkType(String name, int value, Attribute attribute, int ranking) {
    this.name = name;
    this.value = value;
