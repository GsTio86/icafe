...
public class G31DDecoder implements ImageDecoder {
  ...
  public int decode(byte[] pix, int offset, int len) throws Exception {
    ...
    if (totalRunLen < (scanLineWidth * rowsPerStrip)) {
      destByteOffset = outputRunLen(pix, destByteOffset, scanLineWidth * rowsPerStrip - totalRunLen,
          scanLineWidth, 0, len);
    }
  ...
  protected int outputRunLen(byte[] output, int offset, int runLen, int stride, int color,
      int len) {
    int i = 0;
