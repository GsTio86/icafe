...
/**
 * Packbits compression utility class.<p> Contains both unpack and packing method for the Macintosh
 * Packbits compression standard.<p> Contains Variation for 16 bit RLE packing used in PICT
 * files.<p> Contains a main() method with some test code for the pack unpack methods.
 *
 * @author Robin Luiten
...
public class Packbits {
  ...
  /**
   * Variation for PICT - which provides short inb[] for a give outb[] This method when it exhausts
   * input byte buffer it clears the remainder of the output buffer oub.
   **/
  ...
  /**
   * Decompress an RLE compressed buffer of bytes as per format as described in source or in a
   * document on TIFF packbits compression.
   * <p>
   * Worst case for compression is that for every 128 bytes to compress 1 extra byte is required to
   * encode as a copy run. therefore the input buffer should be large enough to accomodate the worst
   * case compression to the output buffer.
   * <p>
   * Throws ArrayStoreException if the array is not large enough for the unpacked data, which could
   * be caused by corrupt input data or the array not be allocated large enough.
   *
   * @param inb  input buffer with compressed data
   * @param outb output buffer for decompressed data the length of the output buffer must be exact
   *             decompressed lenght of compressed input.
   */
   ...
   * @param outb byte buffer to place unpacked byte data to. It is assumed that unpackbits is called
   *             with outb big enough for a single sequence of compressed bytes
   **/
   ...
   * <p>
   * The second part of the code passes through the output compressed bytes and converts any 2 by
   ...
   *             compression of input buffer. [ worst case is 1 additional byte for every 128 bytes
   *             of input buffer with a minimum size of 2 bytes ]
