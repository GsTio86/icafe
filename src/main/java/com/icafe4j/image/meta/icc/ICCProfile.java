 ...
 * WY    30Sep2014  Rewrite to use byte array as input WY    29Sep2014  Added getData() WY
 * 29Sep2014  Added new constructor ICCProfile(byte[])
 */
...
public class ICCProfile extends Metadata {
  ...
  public String getDeviceAttributes() {
    return (isReflective() ? "reflective" : "transparency") + ", " + (isGlossy() ? "glossy"
        : "matte") + ", " + (isPositive() ? "positive" : "negative") + ", " + (isColor() ? "color"
        : "black & white");
  }
    ...
    switch (header.renderingIntent & 0x0000ffff) {
      ...
      default:
        throw new IllegalArgumentException(
            "Unknown rendering intent: " + (header.renderingIntent & 0x0000ffff));
    }
  }
  ...
  public Iterator<MetadataEntry> iterator() {
    ...
    for (TagEntry entry : tagEntries) {
      tagTableEntry.addEntry(
          new MetadataEntry("Tag Name", ProfileTag.fromInt(entry.getProfileTag()) + ""));
  ...
  private static class ICCProfileHeader {
    ...
    private final byte[] preferredCMMType = new byte[4];
    private final byte[] profileVersionNumber = new byte[4];
    private final byte[] colorSpace = new byte[4];
    private final byte[] PCS = new byte[4];
    private final byte[] dateTimeCreated = new byte[12];
    private final byte[] profileFileSignature = new byte[4]; // "acsp" 61637370h
    private final byte[] primaryPlatformSignature = new byte[4];
    private final byte[] profileFlags = new byte[4];
    private final byte[] deviceManufacturer = new byte[4];
    private final byte[] deviceModel = new byte[4];
    private final byte[] deviceAttributes = new byte[8];
    private final byte[] PCSXYZ = new byte[12];
    private final byte[] profileCreator = new byte[4];
    private final byte[] profileID = new byte[16];
    private final byte[] bytesReserved = new byte[28];
