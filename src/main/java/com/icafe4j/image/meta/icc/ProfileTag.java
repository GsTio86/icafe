...
public enum ProfileTag {
  ...
  bXYZ(TagType.PUBLIC, 0x6258595A,
      "blueMatrixColumnTag"), // The third column in the matrix used in matrix/TRC transforms. (This column is combined with the linear blue channel during the matrix multiplication).
  ...
  chad(TagType.PUBLIC, 0x63686164,
      "chromaticAdaptationTag"), // Converts an nCIEXYZ colour relative to the actual adopted white to the nCIEXYZ colour relative to the PCS adopted white. Required only if the chromaticity of the actual adopted white is different from that of the PCS adopted white.
  ...
  clrt(TagType.PUBLIC, 0x636C7274,
      "colorantTableTag"), // Identifies the colorants used in the profile. Required for N-component based Output profiles and DeviceLink profiles only if the data colour space field is xCLR (e.g. 3CLR)
  clot(TagType.PUBLIC, 0x636C6F74,
      "colorantTableOutTag"), // Identifies the output colorants used in the profile, required only if the PCS Field is xCLR (e.g. 3CLR)
  ciis(TagType.PUBLIC, 0x63696973,
      "colorimetricIntentImageStateTag"), // Indicates the image state of PCS colorimetry produced using the colorimetric intent transforms
  cprt(TagType.PUBLIC, 0x63707274, "copyrightTag"), // Profile copyright information
  desc(TagType.PUBLIC, 0x64657363,
  ...
  dmnd(TagType.PUBLIC, 0x646D6E64,
  ...
  gXYZ(TagType.PUBLIC, 0x6758595A,
      "greenMatrixColumnTag"), // The second column in the matrix used in matrix/TRC transforms (This column is combined with the linear green channel during the matrix multiplication).
  ...
  meas(TagType.PUBLIC, 0x6D656173,
      "measurementTag"), // Alternative measurement specification information
  mmod(TagType.PRIVATE, 0x6d6d6f64, "MakeAndModel"),
  ncl2(TagType.PUBLIC, 0x6E636C32,
      "namedColor2Tag"), // PCS and optional device representation for named colours
  ...
  pseq(TagType.PUBLIC, 0x70736571,
  ...
  resp(TagType.PUBLIC, 0x72657370,
      "outputResponseTag"), //	Description of the desired device response
  rigo(TagType.PUBLIC, 0x72696730,
      "perceptualRenderingIntentGamutTag"), // When present, the specified gamut is defined to be the reference medium gamut for the PCS side of both the A2B0 and B2A0 tags
  rig2(TagType.PUBLIC, 0x72696732,
      "saturationRenderingIntentGamutTag"), //	When present, the specified gamut is defined to be the reference medium gamut for the PCS side of both the A2B2 and B2A2 tags
  rXYZ(TagType.PUBLIC, 0x7258595A,
      "redMatrixColumnTag"), // The first column in the matrix used in matrix/TRC transforms. (This column is combined with the linear red channel during the matrix multiplication).
  ...
  tech(TagType.PUBLIC, 0x74656368,
  ...
  ProfileTag(TagType tagType, int value, String description) {
