...
public class TIMEChunk extends Metadata {
  ...
  public TIMEChunk(ChunkType chunkType, int year, int month, int day, int hour, int minute,
      int second) {
  ...
  private static void checkDate(int year, int month, int day, int hour, int minute, int second) {
    if (year > Short.MAX_VALUE || year < Short.MIN_VALUE) {
      throw new IllegalArgumentException(
          "Year out of range: " + Short.MIN_VALUE + " - " + Short.MAX_VALUE);
    }
    if (month > 12 || month < 1) {
  ...
  public Chunk getChunk() {
    if (chunk == null) {
      chunk = new TIMEBuilder().year(year).month(month).day(day).hour(hour).minute(minute)
          .second(second).build();
    }
