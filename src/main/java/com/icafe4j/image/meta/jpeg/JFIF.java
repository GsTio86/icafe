...
public class JFIF extends Metadata {
  ...
  private static void checkInput(int majorVersion, int minorVersion, int densityUnit, int xDensity,
      int yDensity) {
    ...
    if (densityUnit < 0 || densityUnit > 2) {
      throw new IllegalArgumentException(
          "Density unit value " + densityUnit + " out of range [0-2]");
    }
  ...
  public Iterator<MetadataEntry> iterator() {
    ...
    entries.add(new MetadataEntry("Density unit",
        (densityUnit <= 2) ? densityUnits[densityUnit] : densityUnit + ""));
