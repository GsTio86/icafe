...
public class JpegXMP extends XMP {
  ...
  public void write(OutputStream os) throws IOException {
    ...
    for (int j = 0; j < list.getLength(); j++) {
      ...
      if (currentNode.getNodeType() == Node.PROCESSING_INSTRUCTION_NODE && currentNode.getNodeName()
          .equalsIgnoreCase("xpacket")) {
        foundPI = true;
        break;
      }
    }
    ...
    if (extendedXmp != null) { // We have ExtendedXMP
      if (XMLUtils.getAttribute(xmpDoc, "rdf:Description", "xmpNote:HasExtendedXMP").length()
          == 0) {
    ...
    if (extendedXmp != null) { // We have ExtendedXMP
      ...
      for (int i = 0; i < numOfChunks; i++) {
        IOUtils.writeShortMM(os, Marker.APP1.getValue());
        // Write segment length
        IOUtils.writeShortMM(os,
            2 + XMP_EXT_ID.length() + GUID_LEN + 4 + 4 + MAX_EXTENDED_XMP_CHUNK_SIZE);
        // Write segment data
