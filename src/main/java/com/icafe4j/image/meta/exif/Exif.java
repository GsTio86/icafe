 ...
 * WY    09Apr2018  Added getAsString(Tag) to extract value by Tag WY    09Apr2018  Added iterator
 * interface implementation WY    10Apr2015  Moved data loaded checking to ExifReader WY
 * 31Mar2015  Fixed bug with getImageIFD() etc WY    17Feb2015  Added addImageField() to add TIFF
 * image tag WY    11Feb2015  Added showMetadata() WY    03Feb2015  Factored out TiffExif and
 * JpegExif WY    03Feb2015  Made class abstract WY    14Jan2015  Moved thumbnail related code to
 * ExifThumbnail WY    06May2014  Complete rewrite to support adding thumbnail IFD
 */
...
public abstract class Exif extends Metadata {
  ...
  protected com.icafe4j.image.meta.exif.ExifThumbnail thumbnail;
  ...
  public void addExifField(com.icafe4j.image.meta.exif.ExifTag tag, FieldType type, Object data) {
    if (exifSubIFD == null) {
      exifSubIFD = new IFD();
    }
  ...
  public void addGPSField(com.icafe4j.image.meta.exif.GPSTag tag, FieldType type, Object data) {
    if (gpsSubIFD == null) {
      gpsSubIFD = new IFD();
    }
    TiffField<?> field = FieldType.createField(tag, type, data);
  ...
  public String getAsString(Tag tag) {
    ...
    if (tag instanceof TiffTag) {
      ifd = getImageIFD();
    } else if (tag instanceof com.icafe4j.image.meta.exif.ExifTag) {
      ifd = getExifIFD();
    } else if (tag instanceof com.icafe4j.image.meta.exif.GPSTag) {
      ifd = getGPSIFD();
    } else if (tag instanceof com.icafe4j.image.meta.exif.InteropTag) {
  ...
  public com.icafe4j.image.meta.exif.ExifThumbnail getThumbnail() {
    if (thumbnail != null) {
      return new com.icafe4j.image.meta.exif.ExifThumbnail(thumbnail);
    }
   ...
   */
  public void setThumbnail(com.icafe4j.image.meta.exif.ExifThumbnail thumbnail) {
  ...
  public Iterator<MetadataEntry> iterator() {
    ...
    if (containsThumbnail) {
      ...
      thumbnailEntry.addEntry(
          new MetadataEntry("Thumbnail data length", "" + thumbnail.getCompressedImage().length));
  ...
  private void getMetadataEntries(IFD currIFD, Class<? extends Tag> tagClass,
      List<MetadataEntry> items) {
    ...
    } else if (tagClass.equals(com.icafe4j.image.meta.exif.ExifTag.class)) {
      entry = new MetadataEntry("EXIF SubIFD", "EXIF Info", true);
    } else if (tagClass.equals(com.icafe4j.image.meta.exif.GPSTag.class)) {
    ...
    for (TiffField<?> field : fields) {
      ...
      if (tag == com.icafe4j.image.meta.exif.ExifTag.PADDING.getValue()) {
        ftag = com.icafe4j.image.meta.exif.ExifTag.PADDING;
      } else {
        try {
    ...
    if (children.get(TiffTag.EXIF_SUB_IFD) != null) {
      getMetadataEntries(children.get(TiffTag.EXIF_SUB_IFD),
          com.icafe4j.image.meta.exif.ExifTag.class, items);
    }

    if (children.get(TiffTag.GPS_SUB_IFD) != null) {
      getMetadataEntries(children.get(TiffTag.GPS_SUB_IFD),
          com.icafe4j.image.meta.exif.GPSTag.class, items);
    }
  }
      ...
      if (ifds.size() >= 2) {
        ...
        if (field != null) { // JPEG format, save as JPEG
          ...
          if (thumbnailLen > 0) {
            ...
            thumbnail = new com.icafe4j.image.meta.exif.ExifThumbnail(width, height,
                Thumbnail.DATA_TYPE_KJpegRGB, thumbnailData, thumbnailIFD);
          ...
          if (field != null) {
            ...
            thumbnail = new com.icafe4j.image.meta.exif.ExifThumbnail(width, height,
                Thumbnail.DATA_TYPE_TIFF, bout.toByteArray(), thumbnailIFD);
            containsThumbnail = true;
          }
        }
      }
  ...
  public void setThumbnailImage(BufferedImage thumbnail) {
    if (this.thumbnail == null) {
      this.thumbnail = new com.icafe4j.image.meta.exif.ExifThumbnail();
    }
