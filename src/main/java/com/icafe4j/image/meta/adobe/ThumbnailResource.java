 ...
 * WY    09Apr2016  Added new constructor WY    14Apr2015  Fixed a bug with super() call, changed
 * data to null WY    14Apr2015  Added new constructor WY    13Apr2015  Initial creation
 */
...
public class ThumbnailResource extends _8BIM {
  ...
  public ThumbnailResource(ImageResourceID id, int dataType, int width, int height,
      byte[] thumbnailData) {
  ...
  public ThumbnailResource(ImageResourceID id, Thumbnail thumbnail) {
    this(id, thumbnail.getDataType(), thumbnail.getWidth(), thumbnail.getHeight(),
  ...
  private static ImageResourceID validateID(ImageResourceID id) {
    if (id != ImageResourceID.THUMBNAIL_RESOURCE_PS4
        && id != ImageResourceID.THUMBNAIL_RESOURCE_PS5) {
  ...
  private void setThumbnailImage(ImageResourceID id, int dataType, int width, int height,
      int totalSize, byte[] thumbnailData) {
    ...
    if (dataType == Thumbnail.DATA_TYPE_KJpegRGB) {
      ...
      WritableRaster raster =
          Raster.createInterleavedRaster(db, width, height, paddedRowBytes, numOfBands, off, null);
      ColorModel cm =
