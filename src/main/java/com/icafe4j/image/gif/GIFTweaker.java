 ...
 * WY    20Feb2017  Fix splitFrames() throws stream closed exception WY    04Apr2016  Rewrite
 * insertXMPApplicationBlock() to leverage GifXMP WY    09Oct2015  Fixed regression bug with
 * splitAnimatedGIF() WY    16Sep2015  Added insertComment() to insert comment extension WY
 * 17Aug2015  Revised to write animated GIF frame by frame WY    06Jul2015  Added
 * insertXMP(InputSream, OutputStream, XMP) WY    24Jun2015  Renamed splitFramesEx2() to
 * splitAnimatedGIF() WY    30Mar2015  Fixed bug with insertXMP() replacing '\0' with ' ' WY
 * 12Mar2015  Cleaned up debugging console output WY    03Mar2015  Added overloaded
 * insertXMPApplicationBlock() with XMP string as input WY    13Feb2015  Added
 * insertXMPApplicationBlock() to insert XMP meta data WY    13Feb2015  Added code to readMetadata()
 * Comment and XMP meta data WY    20Jan2015  Renamed snoop() to readMetadata() to work with
 * Metadata.readMetadata() WY    28Dec2014  Added snoop() to show GIF image meta data WY
 * 18Nov2014  Fixed bug with splitFramesEx() disposal method "RESTORE_TO_PREVIOUS" WY    17Nov2014
 * Added writeAnimatedGIF(GIFFrame) to work with GIFFrame WY    03Oct2014  Added splitFramesEx2() to
 * split animated GIFs into separate images WY    22Apr2014  Added splitFramesEx() to split animated
 * GIFs into separate images WY    20Apr2014  Added splitFrames() to split animated GIFs into
 * frames
 */
...
public class GIFTweaker {
  ...
  private static boolean copyFrame(InputStream is, OutputStream os, DataTransferObject DTO)
      throws IOException {
  ...
  public static void insertXMPApplicationBlock(InputStream is, OutputStream os, XMP xmp)
      throws IOException {
    // Read and copy header and LSD
    // Create a new data transfer object to hold data
    DataTransferObject DTO = new DataTransferObject();
  ...
  public static void insertXMPApplicationBlock(InputStream is, OutputStream os, byte[] xmp)
      throws IOException {
  ...
  public static void insertXMPApplicationBlock(InputStream is, OutputStream os, String xmp)
      throws IOException {
    insertXMPApplicationBlock(is, os, new GifXMP(xmp));
  }
  ...
  private static void readGlobalPalette(InputStream is, int num_of_color, DataTransferObject DTO)
      throws IOException {
  ...
  private static void readImageDescriptor(InputStream is, DataTransferObject DTO)
      throws IOException {
   ...
   */
  @Deprecated
  public static void splitFrames2(InputStream animatedGIF, ImageWriter writer,
    ...
    int frameCount = 0;
    String baseFileName =
        StringUtils.isNullOrEmpty(outputFilePrefix) ? "frame_" : outputFilePrefix + "_frame_";
    ...
    while (frame != null) {
      ...
      if (frame.getDisposalMethod() == 1
          || frame.getDisposalMethod() == 0) // Leave in place or unspecified
        ; // No action needed
   ...
   * @throws Exception
   */
  public static void splitAnimatedGIF(InputStream animatedGIF, String outputFilePrefix)
      throws Exception {
   ...
   * @throws Exception
   */
  public static void splitAnimatedGIF(InputStream animatedGIF, ImageWriter writer,
      String outputFilePrefix) throws Exception {
    // Create a GIFReader to read GIF frames
    FrameReader reader = new FrameReader();
    // Create a GIFWriter or other writers to write the frames
   ...
   */
  public static void writeAnimatedGIF(BufferedImage[] images, int[] delays, int loopCount,
      OutputStream os) throws Exception {
    writeAnimatedGIF(images, delays, loopCount, ImageParam.getBuilder().applyDither(true).build(),
        os);
  }
   ...
   */
  public static void writeAnimatedGIF(BufferedImage[] images, int[] delays, int loopCount,
      ImageParam param, OutputStream os) throws Exception {
  ...
  public static void writeAnimatedGIF(BufferedImage[] images, int[] delays, OutputStream os)
      throws Exception {
   ...
   * @throws Exception
   */
  public static void writeAnimatedGIF(GIFFrame[] frames, int loopCount, OutputStream os)
      throws Exception {
   ...
   * @throws Exception
   */
  public static void writeAnimatedGIF(GIFFrame[] frames, int loopCount, ImageParam param,
      OutputStream os) throws Exception {
    GIFWriter writer = new GIFWriter();
    writer.setImageParam(param);
    writer.setLoopCount(loopCount);
  ...
  public static void writeAnimatedGIF(List<GIFFrame> frames, int loopCount, OutputStream os)
      throws Exception {
  ...
  public static void writeFrame(GIFWriter writer, OutputStream os, BufferedImage frame)
      throws Exception {
    writer.writeFrame(os, frame);
  }

  public static void writeFrame(GIFWriter writer, OutputStream os, BufferedImage frame, int delay)
      throws Exception {
    writer.writeFrame(os, frame, delay);
  }

  public static void writeFrame(GIFWriter writer, OutputStream os, GIFFrame frame)
      throws Exception {
    writer.writeFrame(os, frame);
  }
