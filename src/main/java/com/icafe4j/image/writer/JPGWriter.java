 ...
 * WY    08Nov2015  Write Adobe APP14 segment for RGB color space WY    21Jun2015  Removed copyright
 * notice from generated JPEG images WY    13Aug2014  Added support for YCCK JPEG image WY
 * 06Aug2014  Added writeAdobeApp14 to support CMYK image WY    04Jun2014  Added ICC_Profile support
 * WY    03Jun2014  Added CMYK image support WY    25Mar2014  Added AAN DCT support WY    22Mar2014
 * Added support for grayScale image WY    12Mar2014  First ever working version
 */
...
public class JPGWriter extends ImageWriter {
  ...
  private final int[] qTableSelector = new int[] {0, 1, 1, 1};
  private final int[][] quant_table = new int[2][];
  private final byte[][][] huffman_bits = new byte[2][2][]; // AC, DC
  private final byte[][][] huffman_values = new byte[2][2][]; // AC, DC
  ...
  protected void write(int[] pixels, int imageWidth, int imageHeight, OutputStream os)
    ...
    writeComment("Created on " + new SimpleDateFormat("yyyy:MM:dd HH:mm:ss z").format(new Date()),
        os);
    ...
    if (grayScale) {
      writeGrayScale(IMGUtils.rgb2grayscale(pixels, imageWidth, imageHeight), os, imageWidth,
          imageHeight);
    } else {
  ...
  private void writeGrayScale(float[][] pixels, OutputStream os, int imageWidth, int imageHeight)
      throws Exception {
  ...
  private void writeFullColor(int[] pixels, OutputStream os, int imageWidth, int imageHeight)
      throws Exception {
    ...
    if (colorSpace == JPGOptions.COLOR_SPACE_YCbCr) {
      ...
      {
        IMGUtils.RGB2CMYK_Inverted(cmykColorSpace, pixels, c[0], c[1], c[2], c[3], imageWidth,
            imageHeight);
      } else {
      ...
      {
        IMGUtils.RGB2YCCK_Inverted(cmykColorSpace, pixels, c[0], c[1], c[2], c[3], imageWidth,
            imageHeight);
      } else {
