 ...
 * WY    21Jun2015  Removed copyright notice from generated PNG images WY    01Mar2014  Added
 * apply_adamptive_filter method
 */
...
public class PNGWriter extends ImageWriter {
  ...
  private static void apply_adaptive_filter(int[] filter_type, byte[] pixBytes, int height,
      int bytesPerPixel, int bytesPerScanLine) {
  ...
  private static void apply_filter(int[] filter_type, byte[] pixBytes, int height,
      int bytesPerPixel, int bytesPerScanLine) {
    //
    for (int j = height - 1, offset = pixBytes.length - bytesPerScanLine; j >= 0;
        j--, offset -= bytesPerScanLine) {
  ...
  private void writeGrayScale(byte[] pixels, int imageWidth, int imageHeight, boolean hasAlpha,
    ...
    // Add IHDR chunk
    IHDRBuilder hdrBuilder =
        new IHDRBuilder().width(imageWidth).height(imageHeight).compressionMethod(0).
    ...
    if (bitsPerPixel != 8) {
      ...
      pixels =
          ArrayUtils.packByteArray(pixels, imageWidth, 0, bitsPerPixel, imageWidth * imageHeight);
    }
  ...
  private void writeIndexed(int[] pixels, int imageWidth, int imageHeight, OutputStream os)
      throws Exception {
        ...
        if (param.getDitherMethod() == DitherMethod.FLOYD_STEINBERG) {
          colorInfo =
              IMGUtils.reduceColorsDiffusionDither(param.getQuantMethod(), pixels, imageWidth,
                  imageHeight, bitsPerPixel, bytePixels, colorPalette);
        } else {
          colorInfo = IMGUtils.reduceColorsOrderedDither(param.getQuantMethod(), pixels, imageWidth,
              imageHeight, bitsPerPixel, bytePixels, colorPalette, param.getDitherMatrix());
        }
      } else {
        colorInfo = IMGUtils.reduceColors(param.getQuantMethod(), pixels, bitsPerPixel, bytePixels,
            colorPalette, true);
      }
    }
    ...
    chunks.add(new IHDRBuilder().width(imageWidth).height(imageHeight).bitDepth(bitsPerPixel)
        .colorType(ColorType.INDEX_COLOR).compressionMethod(0).filterMethod(0).interlaceMethod(0)
        .build());
    ...
    // Pack bytePixels according to bitsPerPixel value
    if (bitsPerPixel != 8) {
      bytePixels = ArrayUtils.packByteArray(bytePixels, imageWidth, 0, bitsPerPixel,
  ...
  private void writeRGB(int[] pixels, int imageWidth, int imageHeight, OutputStream os)
      throws Exception {
