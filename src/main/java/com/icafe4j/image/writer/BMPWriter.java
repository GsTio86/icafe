    ...
    if (param.isApplyDither()) {
      if (param.getDitherMethod() == DitherMethod.FLOYD_STEINBERG) {
        IMGUtils.reduceColorsDiffusionDither(param.getQuantMethod(), pixels, imageWidth,
            imageHeight, 8, newPixels, colorPalette);
      } else {
        IMGUtils.reduceColorsOrderedDither(param.getQuantMethod(), pixels, imageWidth, imageHeight,
            8, newPixels, colorPalette, param.getDitherMatrix());
      }
    } else {
