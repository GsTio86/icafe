 ...
 * WY    14Oct2015  Bug fix for transparent frame WY    05Oct2015  Revised writeFrame() to crop
 * images outside logical screen WY    18Aug2015  Added support to use ImageParam to control dither
 * WY    17Aug2015  Revised to write animated GIF frame by frame WY    17Nov2014  Revised
 * writeGraphicControlBlock() to take more parameters WY    17Nov2014  Added new writeFrame() method
 * to take more parameters WY    16Apr2014  Added writeFrame() to support animated GIF
 */

package com.icafe4j.image.writer;

import static com.icafe4j.image.gif.GIFTweaker.APPLICATION_EXTENSION_LABEL;
import static com.icafe4j.image.gif.GIFTweaker.COMMENT_EXTENSION_LABEL;
import static com.icafe4j.image.gif.GIFTweaker.EXTENSION_INTRODUCER;
import static com.icafe4j.image.gif.GIFTweaker.GRAPHIC_CONTROL_LABEL;
import static com.icafe4j.image.gif.GIFTweaker.IMAGE_SEPARATOR;
import static com.icafe4j.image.gif.GIFTweaker.IMAGE_TRAILER;
...
public class GIFWriter extends ImageWriter {
   ...
   * @throws Exception
   */
  public void prepareForWrite(OutputStream os, int logicalScreenWidth, int logicalScreenHeight)
      throws Exception {
    // Header first
    writeHeader(os, true);
  ...
  protected void write(int[] pixels, int imageWidth, int imageHeight, OutputStream os)
      throws Exception {
    // Write GIF header
    writeHeader(os, true);
   ...
   * @throws Exception
   */
  public void writeAnimatedGIF(BufferedImage[] images, int[] delays, OutputStream os)
      throws Exception {
    // Header first
    writeHeader(os, true);
    ...
    for (int i = 0; i < images.length; i++) {
      ...
      int[] pixels = IMGUtils.getRGB(
          images[i]);//images[i].getRGB(0, 0, imageWidth, imageHeight, null, 0, imageWidth);
  ...
  private void writeFrame(int[] pixels, int imageWidth, int imageHeight, int imageLeftPosition,
      int imageTopPosition, int delay, int disposalMethod, int userInputFlag, OutputStream os)
      throws Exception {
    ...
    if (firstFrame) {
      ...
      writeLSD(os, (short) logicalScreenWidth, (short) logicalScreenHeight, flags, bgcolor,
          aspectRatio);
    ...
    if (firstFrame) {
      ...
      writeImageDescriptor(os, imageWidth, imageHeight, imageLeftPosition, imageTopPosition,
          bitsPerPixel - 1);
  ...
  private void writeFrame(int[] pixels, int imageWidth, int imageHeight, int imageLeftPosition,
      int imageTopPosition, int delay, OutputStream os) throws Exception {
  ...
  private void writeGraphicControlBlock(OutputStream os, int delay, int transparent_color,
      int disposalMethod, int userInputFlag) throws Exception {
    // Scale delay
  ...
  private void writeImageDescriptor(OutputStream os, int imageWidth, int imageHeight,
      int imageLeftPosition, int imageTopPosition, int colorTableSize) throws Exception {
  ...
  // Write logical screen descriptor
  private void writeLSD(OutputStream os, short screen_width, short screen_height, short flags,
      byte bgcolor, byte aspectRatio) throws IOException {
